...
import type { SelectedArea as GlobalSelectedArea, AnalysisType } from "./types"
...
const LOCATIONS: Record<string, {
  ...
  sundarbans: {
    center: [21.9497, 88.9218],
    zoom: 10,
    label: "Sundarbans Delta, WB",
  },
  "sardar sarovar": {
    center: [21.8302, 73.7476],
    zoom: 11,
    label: "Sardar Sarovar, Gujarat",
  },
  hasdeo: {
    center: [22.6, 82.3],
    zoom: 10,
    label: "Hasdeo Forest, Chhattisgarh",
  },
...
function mapAreaToSelectedArea(
  area: SelectedArea,
  beforeDate: string,
  afterDate: string,
  dataSource: string,
  ...
  const ev = area.event
  const lat = Number(area.center[0].toFixed(4))
  const lon = Number(area.center[1].toFixed(4))
  const south = Number(area.bounds.getSouth().toFixed(4))
  const north = Number(area.bounds.getNorth().toFixed(4))
  const west = Number(area.bounds.getWest().toFixed(4))
  const east = Number(area.bounds.getEast().toFixed(4))

  let analysisType: AnalysisType = "vegetation-loss"
  if (ev) {
    const t = ev.type.toLowerCase()
    if (t.includes("water")) analysisType = "water"
    else if (t.includes("urban") || t.includes("construction"))
      analysisType = "urban"
    else if (t.includes("gain") || t.includes("recovery"))
      analysisType = "vegetation-gain"
    else if (t.includes("infrastructure")) analysisType = "infrastructure"
    else analysisType = "vegetation-loss"
  }

  const name = ev ? ev.location : `AOI (${lat}°N, ${lon}°E)`
  const state = ev?.location.includes(",")
    ? ev.location.split(",")[1].trim()
    : "India"
  const affectedArea = ev
    ? ev.area
    : `~${Math.abs((north - south) * 111 * (east - west) * 111 * Math.cos((lat * Math.PI) / 180)).toFixed(1)} km²`
  const confidence = ev ? ev.confidence : 92
  const sensor =
  ...
  return {
    id: ev ? ev.id : `MAP-${Math.round(lat * 100)}-${Math.round(lon * 100)}`,
    name,
    state,
    latitude: lat,
    longitude: lon,
    ...
    analysisType,
    changeDescription: ev ? ev.type : "Satellite Change Detection",
    beforeDate,
    afterDate,
    sensor,
    confidence,
    affectedArea,
    explanation: `Multi-temporal change detected in ${name}. Satellite observation window from ${beforeDate} to ${afterDate}.`,
  }
}
...
export default function MapPage({
  ...
  onNavigate: (p: string, area?: GlobalSelectedArea) => void
  activeArea?: GlobalSelectedArea
}) {
  ...
  const [flyTarget, setFlyTarget] = useState<{ center: [number, number]; zoom: number } | null>(() =>
    activeArea && activeArea.latitude && activeArea.longitude
      ? { center: [activeArea.latitude, activeArea.longitude], zoom: 11 }
      : null,
  )
  ...
  useEffect(() => {
    if (activeArea && activeArea.latitude && activeArea.longitude) {
      setFlyTarget({
        center: [activeArea.latitude, activeArea.longitude],
        zoom: 11,
      })
      if (activeArea.beforeDate) {
        setBeforeDate(activeArea.beforeDate)
      }
      if (activeArea.afterDate) {
        setAfterDate(activeArea.afterDate)
      }
    }
  }, [activeArea])
  ...
  const handleLocationSelect = (name: string) => {
    const key = Object.keys(LOCATIONS).find(
      (k) =>
        ...
        k.includes(name.toLowerCase().split(",")[0].trim()),
    )
  ...
  const handleNavWithArea = (pageName: string) => {
    const target = selectedArea
      ? mapAreaToSelectedArea(selectedArea, beforeDate, afterDate, dataSource)
      : activeArea
    onNavigate(pageName, target)
  }
        ...
        <div className="flex-1 relative overflow-hidden">
          {comparisonMode === "side-by-side" ? (
            ...
            <MainMap
              ...
              changeEvents={
                activeOverlays.changeDetection ? MOCK_CHANGE_EVENTS : []
              }
            />
          )}
          ...
          <MapHUD
            coords={coords}
            zoom={zoom}
            basemap={tileConfig.basemap}
            analysis={tileConfig.analysis}
          />
              ...
              <div className="flex gap-sm">
                {(["side-by-side", "swipe"] as const).map((m) => (
                  <button
                    key={m}
                    onClick={() =>
                      setComparisonMode(comparisonMode === m ? "none" : m)
                    }
  ...
  return (
    ...
    >
      ...
      <MapEventHandler
        ...
        onAreaClick={(center, bounds) => onAreaSelect({ bounds, center })}
      />
    ...
    <div className="flex h-full">
      ...
      <div className="flex-1 relative overflow-hidden">
        ...
        <MapContainer
          ...
          zoomControl={false}
        >
          <TileLayer url={tileConfig.url} attribution="" maxZoom={19} />
          <FlyController target={flyTarget} onComplete={onFlyComplete} />
        </MapContainer>
      </div>
    ...
    >
      ...
      >
        <MapContainer
          ...
          dragging={false}
        >
          <TileLayer url={tileConfig.url} maxZoom={19} attribution="" />
          <FlyController target={flyTarget} onComplete={onFlyComplete} />
        </MapContainer>
      </div>
      ...
      {/* Labels */}
      <div
        className="absolute top-3 z-[400]"
        style={{ left: `${swipePos / 2}%`, transform: "translateX(-50%)" }}
      >
    ...
    <div className="absolute left-4 bottom-4 z-[500]">
      <div className="bg-surface-dark/90 backdrop-blur-sm rounded-corner-lg border border-border-secondary/30 overflow-hidden">
        ...
        >
          ...
          <span className="text-video-title text-on-brand font-medium">
            Legend
          </span>
          <ChevronDown
            size={12}
          ...
          <div className="px-md pb-md flex flex-col gap-xs border-t border-border-secondary/20 pt-sm">
            {legendItems.map(({ color, label }) => (
              <div key={label} className="flex items-center gap-sm">
                <div
                  className={`w-2.5 h-2.5 rounded-xs flex-shrink-0 ${color} opacity-80`}
                />
                <span className="text-video-title text-on-brand/80">
                  {label}
                </span>
              </div>
            ))}
...
function SelectedAreaPanel({
  ...
  const ev = area.event
  const mappedArea = mapAreaToSelectedArea(
    area,
    beforeDate,
    afterDate,
    dataSource,
    ...
    <div className="w-72 bg-surface-bg border-l border-border-secondary flex flex-col z-[500] relative overflow-y-auto">
      ...
      <div className="flex-1 p-xl flex flex-col gap-xl">
            ...
            <div className="flex flex-col gap-sm">
              {[
                ...
                { label: "Urban Expansion", active: false, icon: BarChart3 },
                {
                  ...
                  active: ev.type === "Water Level Change",
                  icon: Globe,
                },
                ...
                >
                  ...
                  <span
                    className={`text-label-sm ${
                      active ? "text-danger font-medium" : "text-text-tertiary"
                    }`}
                  >
                    {label}
                  </span>
        ...
        <div className="bg-bg-faint rounded-corner-md p-lg flex flex-col gap-sm">
          ...
          {[
            ...
            [
              "Area",
              `~${Math.abs((area.bounds.getNorth() - area.bounds.getSouth()) * 111 * (area.bounds.getEast() - area.bounds.getWest()) * 111 * Math.cos((area.center[0] * Math.PI) / 180)).toFixed(1)} km²`,
            ],
          ].map(([k, v]) => (
      ...
      <div className="p-xl border-t border-border-secondary flex flex-col gap-md flex-shrink-0">
        <Button
          variant="primary"
          iconStart={<Activity size={16} />}
          onClick={() => onNavigate("change-detection", mappedArea)}
        >
          Analyse This Area
        </Button>
        <Button
          ...
          onClick={() => onNavigate("report", mappedArea)}
        >
          Generate Report
        </Button>
        <Button
          ...
          onClick={() => exportAreaToGeoJSON(mappedArea)}
        >
          Export GeoJSON
        </Button>
      </div>
